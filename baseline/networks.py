...
class ResNet50(nn.Module):
    def __init__(self, pretrained=True):
        ...
        self.model = resnet101(pretrained=False, output_stride=16)
...
class ResNet101(nn.Module):
    ...
    def forward(self, x):
        ...
        out = nn.AvgPool2d(x4.shape[2:])(x4).view(x4.shape[0], -1)
...
class PANet(nn.Module):
    def __init__(self, num_classes=1):
        ...
        self.convnet = ResNet50(pretrained=False)
        self.classifier = Classifier(in_features=2048, num_class=num_classes)
        self.pan = PAN(self.convnet.blocks[::-1])
        self.mask_classifier = Mask_Classifier(in_features=256, num_class=num_classes)

    def forward(self, x):
        fms_blob, z = self.convnet(x)
        out_cls = self.classifier(z.detach())

        out_ss = self.pan(fms_blob[::-1])
        mask_pred = self.mask_classifier(out_ss)
        out_prd = F.interpolate(mask_pred, scale_factor=4, mode='nearest')
        return out_cls, out_prd


class PyAtNet(nn.Module):
    def __init__(self, num_classes=1, pretrained=False):
        super(PyAtNet, self).__init__()
        # self.model = resnet.resnet50(pretrained=pretrained)
        self.model = resnet101(pretrained=False, output_stride=16)
        self.relu = nn.ReLU(inplace=True)
        layers_cfg = [4, 5, 6, 7]
    ...
    def forward(self, x):
        ...
        out_cls = self.fc1(out.detach())

        out_ss = self.pan(feature_map[::-1])
        mask_pred = self.mask_classifier(out_ss)
        out_prd = F.interpolate(mask_pred, scale_factor=4, mode='bilinear')
        return out_prd, out_cls


if __name__ == '__main__':
    # model = PyAtNet()
    # input_tensor = torch.rand(2, 3, 512, 512)
    # os, od = model(input_tensor)
    # print(od.shape)
    # summary(model.cuda(), input_size=(3, 512, 512), batch_size=1)

    from modules.modelsummary import get_model_summary
