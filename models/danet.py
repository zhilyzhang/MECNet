...
class BaseNet(nn.Module):
    def __init__(self, nclass, backbone, aux, se_loss, dilated=True, norm_layer=None, **kwargs):
...
class DANet(BaseNet):
    ...
    def __init__(self, nclass=1, backbone='resnet101', aux=False, se_loss=False, norm_layer=nn.BatchNorm2d, **kwargs):
    ...
    def forward(self, x):
        ...
        x = self.head(c4)
        x = list(x)
        x[0] = upsample(x[0], imsize, mode='bilinear')
        x[1] = upsample(x[1], imsize, mode='bilinear')
        x[2] = upsample(x[2], imsize, mode='bilinear')
...
if __name__ == '__main__':
    band_num = 3
    class_num = 1
    model = DANet(nclass=1, backbone='resnet101')
